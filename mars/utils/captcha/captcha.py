...
class Captcha(object):
    def __init__(self):
        ...
        self._text = random.sample(string.ascii_uppercase + string.ascii_uppercase + '3456789', 4)
        self.fonts = [os.path.join(self._dir, 'fonts', font) for font in ['Arial.ttf', 'Georgia.ttf', 'actionj.ttf']]
        self.width = 200
        self.height = 75
        self._color = self.random_color(0, 200, random.randint(220, 255))
    ...
    def captcha(self, path=None, fmt='JPEG'):
        ...
        img_name = "".join(random.sample(string.ascii_lowercase + string.ascii_uppercase + '3456789', 24))
        text = "".join(self._text)
        out = BytesIO()

        image.save(out, format=fmt)
        if path:
            image.save(os.path.join(path, img_name), fmt)
...
if __name__ == '__main__':
    name, vcode, content = captcha.generate_captcha()
    print(vcode)
